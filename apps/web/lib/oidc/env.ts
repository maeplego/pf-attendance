...
export function attendanceApiBase(): string {
  const v =
    process.env.ATTENDANCE_API_URL?.replace(/\/$/, "") ||
    process.env.NEXT_PUBLIC_ATTENDANCE_API_URL?.replace(/\/$/, "");
  return v || "http://localhost:8019";
}
